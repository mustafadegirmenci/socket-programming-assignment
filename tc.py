...
def apply_packet_delay_normal(delay, jitter, interface=DEFAULT_INTERFACE):
    ...
    command = f"tc qdisc add dev {interface} root netem delay {delay} distribution normal"
    subprocess.run(command, shell=True)
...
def apply_packet_delay_uniform(delay, jitter, interface=DEFAULT_INTERFACE):
    ...
    command = f"tc qdisc add dev {interface} root netem delay {delay} distribution uniform"
    subprocess.run(command, shell=True)
